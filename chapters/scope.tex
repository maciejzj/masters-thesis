...
It is best to use it inside a Python \textit{virtualenv}, running command: \mintinline[breaklines]{shell}{pip install -r requirements.txt}.
...
Sine the metadata is versioned with Git, when going back in commit history one should run \mintinline{shell}{dvc checkout} after \mintinline{shell}{git checkout} to conceal data versions.
