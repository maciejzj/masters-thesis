...
\begin{figure}
        \centering
                 \begin{tikzpicture}
		 \begin{axis}[width=\textwidth, height=4cm, grid=major, grid style={dashed}, xmin=0.2, xmax=1, ytick={1, 2}, yticklabels={High--res, Low--res}]
		      \addplot+ [boxplot prepared={draw position=1,
		      							   average=0.47,
		                                   median=0.44,
		                                   lower whisker=0.31,
...
There are various ways to upscale image during the decoding process.
...
\begin{sidewaysfigure}
    ...
    \hfill
        \centering
        \begin{tikzpicture}
			\begin{axis}[grid=major, grid style={dashed}]
...
Training a \textsc{gan} can pose a significant challange when stopping condition is taken into consideration.
...
\begin{figure}
    \centering
    \begin{tikzpicture}
			\begin{axis}[width=0.75\linewidth, height=8cm, grid=major, grid style={dashed}]
...
Figure \ref{fig:intermediate-results} presents an example of image shrinking using various methods, figure \ref{fig:intermediate-results-zoomed} presents enlarged part of the same image.
The visual comparison has been created using simple convolutional augmentation network.
